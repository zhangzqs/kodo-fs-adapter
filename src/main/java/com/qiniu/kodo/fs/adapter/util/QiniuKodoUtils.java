...
public final class QiniuKodoUtils {
     ...
     */
    public static Path keyToPath(Path workingDir, String key) {
        return new Path(Path.PATH_SEPARATOR + key).makeQualified(workingDir);
    }
    ...
    public static String keyToFileKey(String key) {
        if (key == null || !key.endsWith(Path.PATH_SEPARATOR)) {
            return key;
        }
        return key.substring(0, key.length() - Path.PATH_SEPARATOR.length());
    }
    ...
    public static String keyToDirKey(String key) {
        if (key == null || key.length() == 0 || key.endsWith(Path.PATH_SEPARATOR)) {
            return key;
        }
        return key + Path.PATH_SEPARATOR;
    }
    ...
    public static boolean isKeyDir(String key) {
        ...
        return key.endsWith(Path.PATH_SEPARATOR);
    }
}
